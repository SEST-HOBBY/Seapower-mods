...
Everything else is held fixed on purpose, including the SM-6 and Stunner
altitude fixes and the VeryLarge impact tier. If B dropped those too it would
differ from A in four ways and no result would be attributable.
...
def main():
    ...
    print("rounds that inherit BOTH penalties. The SM-3 is NOT one of them: SEST_Aegis_BMD")
    print("gives it explicit values, so it cannot probe what a missing global falls back to.")
