...
Exits non-zero if a pack hangs a store, rosters a unit or names a system
that nothing defines (a pruned or never-exported provider) and that no
upstream copy of the file already carried, or depends on a mod missing from
the load order. What a forked file inherited from its upstream is listed, not
failed.
"""
import collections
import re
...
def system_index():
    ...
    SystemName that nothing defines is a sensor that does not exist - and
    until this checked them, a pack that leaned on another mod only for a
    system was reported standalone. SEST_ADF_Persistent_ISR was: its Triton's
    AN/AAS-52_Visual turret is defined by the MQ-9 Reaper mod (3503670861)
    and nowhere else.
    """
...
def upstream_store_names():
    ...
    is a bug here. Without the distinction, a pack that forks an upstream hull
    to fix one launcher would fail over a store the upstream never shipped -
    a reference that is equally broken with or without SEST - and the only way
    back to green would be to drop the fix.
    """
    names = set()
...
def upstream_roster_names():
    ...
    roster is this repo's by construction. The case the rule exists for is a
    fork of a hull whose upstream removed the hangar and kept the roster line:
    the fork inherits both with the file, and neither is this repo's doing.
    """
...
def main():
    ...
    if inherited_rosters:
        where = len({r for rels in inherited_rosters.values() for r in rels})
        print(f"\n{len(inherited_rosters)} unit id(s) rostered by {where} forked file(s) "
              "resolve to nothing anywhere.")
        print("   The upstream copy of the same file rosters them too - that squadron "
              "never spawns,")
        print("   with or without SEST.")
        for name in sorted(inherited_rosters):
            rels = sorted(inherited_rosters[name])
            more = f" (+{len(rels) - 2} more)" if len(rels) > 2 else ""
            print(f"      {name:<28} "
                  f"{', '.join(Path(r).stem for r in rels[:2])}{more}")
