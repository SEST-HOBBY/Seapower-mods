...
harm classification behind its "three rounds need fixing" claim. Committed so
the next mods-source export can re-run it rather than take those numbers on
trust: a new mod, or a reorder, can move a band into the danger set without any
file this repo owns changing - which is how rok_k-sam-II joined the list.
...
Two things it does that a plain grep cannot:
  ...
  - Directive inheritance, both kinds. 115 winning ammunition files open with
    ...
    misses 23 anti-air rounds and miscounts the one-sided total.
...
def resolve(rel, chain=()):
    ...
    An #!alias that names its own path (Euromod's rfn_3m55 does) is read the
    way tools/check_alias_bases.py reads it: as a layer on the copy below, like
    an extend. Resolved against the top of the order it would name itself, and
    this used to count it as an alias cycle.
    ...
    if m:
        ...
        own_path = target.replace("\\", "/").lower() == rel.lower()
        base = (resolve(target, chain + (rel,)) if kind == "alias" and not own_path
...
def main():
    ...
    print("  of which #!alias/#!extend stubs ", stats["alias"])
    print("    of those, #!extend            ", stats["extend"])
    ...
    print(f"\nPROBE ROUNDS for the missing-global test: {len(probes)} anti-air rounds\n"
          ...
          "out-of-altitude clamp. Do NOT use the SM-3 for this - it declares both\n"
          "penalties itself (SEST_Collection_Fixes sets them to 0.01 and 0.10), so it\n"
          "inherits neither. A sample:")
    for r in probes[:12]:
        print(f"   {r['id']:28} {r['winner']:16} "
    ...
    print("\nRead the winner column: this resolves the tree INCLUDING the SEST packs,\n"
          ...
          "and a defect already fixed reads clean. usn_rim_174a/b/c, rok_k-sam-II and\n"
          "idf_stunner are the five SEST_Intercept_Model owns - to see what upstream\n"
          "still ships, read their donors under mods-source/ directly.")
