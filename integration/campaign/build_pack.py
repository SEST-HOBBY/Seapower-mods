...
def campaign_ini(missions, events, placements):
    ...
    for i, entry in enumerate(entries, 1):
        ...
        if entry["type"] == "Mission":
            ...
            # The card the campaign map draws for this mission. Generated by
            # make_art from the mission that was just written, so it cannot
            # describe a mission that changed underneath it.
            L.append(f"MissionImage_en={sheet_path(entry['mission'])}")
        else:
            L.append(f"AssetsPath_en=campaigns/{SLUG}/art")
            L.append(f"FilePath_en=campaigns/{SLUG}/art/{entry['file']}.xml")
            L.append(f"TileImagePath_en=campaigns/{SLUG}/art/"
                     f"{entry['file']}_image.png")
        L.append("")
    return "\n".join(L)
