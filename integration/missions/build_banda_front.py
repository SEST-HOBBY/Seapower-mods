...
Compared with SEST Indo-Pacific Land Assets:
  ...
  - Modern airliners in regional liveries cross the zone on real city pairs,
    ...
    Neira, the PNG highlands, the Timor Sea rigs, the Mahakam delta). Every
    route is then carried on along its airway by extend_civil_airways.py, so
    nothing circles at its last waypoint.
...
import extend_civil_airways                                      # noqa: E402
...
def main():
    ...
    # The short hops end over the sea, where an aircraft at its last waypoint
    # orbits; carry each one on along its airway, as every mission here is.
    extended = extend_civil_airways.fix_file(OUT, write=True)
    print(f"  civil aircraft routes extended along their airway: {len(extended)}")
