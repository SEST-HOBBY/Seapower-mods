...
Name= is the output name, and re-running gives the same bytes. An existing
output whose bytes differ from this run's is not replaced without --force:
it may carry edits made since it was derived. That is why the default output
is not "SEST Banda Front Lean v2" any more - Lean v2 was derived this way,
then edited in the game (submarine patrols, a supply ship, moved placements)
and given back its rules of engagement, so re-deriving it would lose all of
that.

    python3 integration/missions/thin_waypoints.py                   # Living Seas -> Living Seas thinned, narco subs and fishing boats
...
OUT = "SEST Banda Front Living Seas thinned"
...
def parse_args():
    ...
    ap.add_argument("--force", action="store_true",
                    help="replace an existing output even if its bytes differ from this run's")
...
def main():
    ...
    if desc and "one lap" not in desc.group(1):
        mission.set_description(desc.group(1).rstrip(". ") + ". Patrol loops cut to one lap.")
    problems = mission.verify()
    if problems:
    ...
    data = mission.text().encode("utf-8")
    # A re-run may replace its own output; a file edited since, never silently.
    stale = out.exists() and out.read_bytes() != data
    if a.dry_run:
        if stale and not a.force:
            print(f"{out.name} exists and differs from this result: writing it would need --force")
        print("dry run - nothing written")
        return
    if stale and not a.force:
        sys.exit(f"{out.name} exists and differs from this result - it may carry edits made "
                 f"since it was derived. Pick another --out, or pass --force to replace it.")
