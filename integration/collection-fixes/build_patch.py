...
the eight Nimitz Expanded carriers with the CVN-70 typo fixed, restores the \
Nimitz (2000s) CVN-70 Carl Vinson variant and carries that carrier's display \
names, and repairs the MH-60R's dangling Penguin seat reference. Generated by \
...
def main():
    ...
    # usn_cvn_nimitz_2000s_variants [audit: usn_cvn_nimitz_2000s cohort]. Murder
    # Hornet (3430135740) wins the (2000s) Nimitz pair and declares
    # NumberOfVariants=2 while shipping a complete [Variant3] block (CVN-70
    # Carl Vinson, "#Not included atm"). The engine pools only the declared
    # count, so the block is present yet unselectable: AUS DEF asks for
    # Variant3 and the editor shows "MISSING: usn_cvn_nimitz_2000s name or
    # squadron reference" for it. Declared count -> 3 (the audit's first
    # recommended fix). Variant3's art wiring is taken from Nimitz Expanded's
    # CVN-70 block, whose hull-number and emblem textures ship with that mod;
    # the donor pointed the hull number at ui/number/, a folder nothing ships.
    # The air groups are the donor's, untouched.
