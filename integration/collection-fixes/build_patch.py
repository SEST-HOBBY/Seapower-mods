...
SM-3, SM-6 and PAC-3 MSE had commented out, brings the SM-3's \
unreachable 220,000-ft target floor down to 150,000 ft, where the \
ballistic threats it exists to intercept actually fly, and gives the SM-3 \
the 100 nm / 90 deg seeker, 50 nm terminal handover and launch profile its \
author ships in the Euromod Anchorchain Expansion, so it homes the same \
with or without the Anchor Chain preloader. Generated by \
...
# --------------------------------------------------- SM-3 overwrite fold
# The Euromod SM-3s (usn_rim-161b/c/d) keep their guidance in two places.
# The base file in 3629144864 carries a stub: an 18 s unguided first phase
# (14 s on the D), an 8 nm terminal handover, a 10 nm / 30 deg IR seeker,
# and LaunchTurnRate and TimeLimited commented out. The author's real values
# sit in the Euromod Anchorchain Expansion (3784474738), in
# ammunition_overwrite/usn_rim-161*_OVWR.ini - and that folder is read only
# by the Anchor Chain preloader, whose install data/mod-catalog.json records
# as unverified (known_missing_dependencies). Without it the round homes on
# the stub: a seeker that sees 10 nm and takes over from midcourse at 8.
#
# So the overwrite's flight model is folded into the base file, and the
# round flies the same with or without the preloader. Every value below is
# the overwrite's; nothing is invented. The builder re-reads the overwrite
# on every build and fails if the author changes anything in it that bears
# on flight - a copied value, a declined one, one that agrees with the base
# file, or a key it did not set before.
#
# MaxLoftAngle is 90.0, the overwrite's value, not the 85.0 on the line the
# author commented out in the base file (which this pack un-commented
# first). Three reasons. With the preloader the overwrite puts 90.0 back on
# top of whatever this pack ships, so 85.0 would leave the one flight-model
# key on which the two configurations still differ. The overwrite has
# carried 90.0 and LaunchTurnRate=5 together since the first export: a Mk 41
# cell fires straight up, and a vertical loft asks nothing of a launch turn
# that slow. And the SM-6 overwrites repeat their base files' commented
# 80.0, so the SM-3's different figure reads as a revision, not a slip.
# The difference in flight is small - a 5 deg lean is under 3 nm of ground
# in a climb to 200,000 ft - and 90.0 is a value the engine flies: a dozen
# other shipped files use it, among them Euromod's own ger_3DDS and
# usn_ircps and the collection's SLBMs (JL-3, R-29RMU, Trident, M51).
SM3_OVERWRITE = "3784474738"
SM3_OVWR_VALUES = {
    ...
    # Commented out in the base file; the B has no TimeLimited line at all.
    "LaunchTurnRate": "5",
    "TimeLimited": "True",
}
...
# flight times to each block's declared range (600/600/900, main() below)
# rather than a figure that would let the round fly for fifty minutes on
# every install. With the preloader the overwrite still applies on top,
# and the D then flies 3000 s whatever this pack ships.
SM3_OVWR_DECLINED = {
    "MaxFlightTime": {"usn_rim-161d": "3000"},
}
...
def ini_values(text):
    """Every live key=value in an .ini, as {key: [values]}, comments cut."""
    out = {}
    for ln in text.splitlines():
        body = ln.split("//", 1)[0]
        if "=" in body and ln.strip()[:1] not in ("#", ";", "["):
            key, value = body.split("=", 1)
            out.setdefault(key.strip(), []).append(value.strip())
    return out


def sm3_check_overwrite(t, name):
    """Fail unless the overwrite still reads exactly as SM3_* records it."""
    ovwr = ini_values(read(SM3_OVERWRITE, f"ammunition_overwrite/{name}_OVWR.ini"))
    base = ini_values(t)
    ...
    for key, value in SM3_OVWR_VALUES.items():
        if ovwr.get(key) != [value]:
            sys.exit(f"{name}: the Anchorchain overwrite now has {key}="
                     ...
                     "flies differently with and without the preloader")
    for key, per_round in SM3_OVWR_DECLINED.items():
        want = [per_round[name]] if name in per_round else None
        if ovwr.get(key) != want:
            sys.exit(f"{name}: the Anchorchain overwrite now has {key}="
                     f"{shown(ovwr.get(key))} where {shown(want)} was recorded - "
                     f"{rebase}")
    for key in SM3_OVWR_SAME:
        if ovwr.get(key) != base.get(key):
            sys.exit(f"{name}: the Anchorchain overwrite now has {key}="
                     f"{shown(ovwr.get(key))}, the base file {shown(base.get(key))} - "
                     f"decide which the round should have and {rebase}")
    known = set(SM3_OVWR_VALUES) | set(SM3_OVWR_DECLINED) | set(SM3_OVWR_SAME)
    new = sorted(k for k in ovwr if k not in known and not SM3_OVWR_IGNORABLE.search(k))
    if new:
        sys.exit(f"{name}: the Anchorchain overwrite now also sets {', '.join(new)}. "
                 "Copy each into SM3_OVWR_VALUES or record it in SM3_OVWR_DECLINED "
                 f"with a reason - {rebase}")


def sm3_fold_overwrite(t, name):
    """Fold the overwrite's flight model into the base file (see SM3_*).
    Returns the text and the header lines that say so."""
...
def main():
    ...
    for name, angle in LOFT_ANGLE.items():
        ...
        if name in SM3:
            ...
            # 4. IT HOMES ON A STUB. The seeker, handover and launch profile
            #    are the author's, from the Anchorchain overwrite, so they no
            #    longer depend on the preloader (sm3_fold_overwrite above).
            t, fold = sm3_fold_overwrite(t, name)
            extra += fold
