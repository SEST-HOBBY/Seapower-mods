...
the eight Nimitz Expanded carriers with the CVN-70 typo fixed, carries the \
Nimitz (2000s) display names so that carrier keeps its type and hull names \
when the mod that shipped them goes, and repairs the MH-60R's dangling Penguin seat reference. Generated by \
...
def main():
    ...
    # usn_cvn_nimitz_2000s_variants [audit: usn_cvn_nimitz_2000s cohort].
    #
    # WITHHELD 20 Sep 2026 - this override is the only functional change the
    # collection pack made between a load that worked and one that hung. It is
    # off until the game log says otherwise; re-enabling is the flag below.
    #
    # What it did: Murder Hornet (3430135740) wins the (2000s) Nimitz pair and
    # declares NumberOfVariants=2 while shipping a complete [Variant3] block
    # (CVN-70 Carl Vinson, marked "#Not included atm" by its own author). The
    # engine pools only the declared count, so the block is present yet
    # unselectable: AUS DEF asks for Variant3 and the editor shows "MISSING:
    # usn_cvn_nimitz_2000s name or squadron reference". This raised the count
    # to 3 and spliced in Nimitz Expanded's (3432592449) CVN-70 art wiring.
    #
    # Why it is the suspect. Raising the count makes the engine pool a variant
    # nothing pooled before, and that variant's three textures are named with
    # .png suffixes under aircraft/materials/emblem[s]/ - where Variant1 and
    # Variant2 in the same file use bare names under ships/materials/emblem/.
    # The art is a faithful copy of Nimitz Expanded's block, but those textures
    # ship with THAT mod while the unit is won by Murder Hornet, and whether
    # they resolve across that boundary was never demonstrated: mods-source
    # carries .ini files only, so no check in this repo can see a texture. A
    # newly pooled variant whose art misses is a credible long stall, and
    # vessels/ is a whole-file override at tier 0, so it applied to every
    # mission, not just the one that names the carrier.
    #
    # What re-enables it: a player.log from a run with it on that shows the
    # load completing, or that names a different culprit. The cost of leaving
    # it off is one carrier variant in one mission, unselectable exactly as it
    # was before this pack existed.
    BUILD_NIMITZ_2000S_VARIANT3 = False
    if BUILD_NIMITZ_2000S_VARIANT3:
        ctx = "usn_cvn_nimitz_2000s_variants"
        t = read("3430135740", "vessels/usn_cvn_nimitz_2000s_variants.ini")
        t = edit(t, r"^NumberOfVariants=2$", "NumberOfVariants=3", 1, ctx)
        art_donor = re.search(r"^\[Variant3\][^\n]*\n(.*?)\n\s*\n",
                              read("3432592449", "vessels/usn_cvn_nimitz_variants.ini"),
                              re.S | re.M)
        if not art_donor or not all(k in art_donor.group(1) for k in
                                    ("HullnumberTexture=usn_cvn_070", "EmblemTexture=cvn-70",
                                     "AircraftEmblemTexture=cvn-70", "FlagTexture=flag_us",
                                     "Nation=US")):
            sys.exit(f"{ctx}: Nimitz Expanded's [Variant3] art block changed - rebase")
        if "CustomAirGroup" in art_donor.group(1):
            sys.exit(f"{ctx}: Nimitz Expanded's [Variant3] art block now runs into its "
                     "air group - rebase")
        # Donor art block = the header through its Nation=US line; a callable
        # replacement keeps the donor text literal (no backslash re-parsing).
        t = edit(t, r"^\[Variant3\]\s*#Not included atm\s*\n(?:[^\n]*\n)*?Nation=US\n",
                 lambda _m: "[Variant3]\n" + art_donor.group(1) + "\n", 1, ctx)
        if "ui/number/" in t:
            sys.exit(f"{ctx}: donor's ui/number/ hull-number path survived the splice - rebase")
        write("vessels/usn_cvn_nimitz_2000s_variants.ini", t,
              "SEST Collection Fixes - base: 3430135740's (2000s) Nimitz variants table.\n"
              "Two deltas: NumberOfVariants 2 -> 3, so the CVN-70 Carl Vinson block the\n"
              "donor already ships is selectable (AUS DEF names it), and Variant3's\n"
              "hull-number/emblem wiring taken from Nimitz Expanded (3432592449), whose\n"
              "CVN-70 textures exist; the donor's ui/number/ path pointed at nothing.")
        built.append(ctx)
