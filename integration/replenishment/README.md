...
fleet: **309 modern hulls** carry launchers that **can never be reloaded by anything**,
...
categories on *flight decks* only, so in vanilla `SovietAdvancedASM` is a total orphan, stocked
by no supplier at all.
...
out on the Long Beach's Mk141 Harpoon canisters, which set it `False`. Across vanilla and every
exported mod that flag is set `True` **only on land SAM TELs**. No vessel anywhere sets it.
...
of the 103 hulls that carry one could ever be replenished.
...
| | Launchers | |
|---|---:|---|
| No `Ammunition=` at all | 4 533 | guns fed from a magazine, decoy dispensers, empty loadout stubs — not affected |
| `Ammunition=` **and** an `AssociatedMagazine=` | 173 | reloads today, exactly as vanilla intends |
| **Bare `Ammunition=`, no magazine, no flag** | **2 220** | **one-shot forever** |

So of the launchers that actually hold a round, **2 220 of 2 393 — 93% — cannot be
replenished by anything**. They sit on 309 hulls:

| Mod | Hulls | Launchers |
|---|---:|---:|
| Red Storm Arsenal | 103 | 1 185 |
| PLAN Pack | 25 | 275 |
| Modern US Navy | 61 | 257 |
| Russian Navy 21 | 12 | 117 |
| Euromod — South Korea Navy | 11 | 64 |
| Euromod — Modern German Navy | 17 | 52 |
| Euromod — Modern Italian Navy | 7 | 40 |
| Euromod — Modern British Navy | 11 | 37 |
| Euromod — Modern Dutch Navy | 7 | 35 |
| *15 further mods* | 55 | 158 |
...
This pack flags **2 215 launchers on 307 of those hulls — every one it is entitled to touch**.
The remaining 5 sit on two hulls owned by sibling SEST packs (`js_ffg_mogami` in SEST JMSDF
Mogami, `rn_lph_ocean` in SEST Allied Fixes), which apply the same fix in their own builders.
...
| **Repairs** | 6 broken upstream ammunition references fixed on 7 hulls — launchers that had no round |
| **Launchers** | **2 217 launchers made reloadable** on 308 hulls from 23 mods (24 Sep 2026 export) |
...
*What the pack cannot ration at all.* Gate 5 is this pack's doing — before it, 2 217
launchers could not be reloaded by anything. **349 of them hold a round carrying neither
`AmmoPoints` nor a `SupplyCategory`**, and for those no gate can ration the transfer: gate 3
compares against a price that is not there, gate 4 against a category that is not there. 37
rounds are behind that number, and the heavy end of it is real — `plan_yj-18a` (50
launchers), `wp_ss_n_27` (Klub, 32), `wp_ss_n_26a` (Oniks, 30), `plan_yj-20` (17),
`plan_yj12_ship` (11), and the `wp_r-39` SLBM tubes, which the flag makes reloadable at sea.
...
That comes to **87 rounds** on the 24 Sep 2026 export (56 land-attack, 31 area-SAM) and 8
deliberate exclusions, all of them land-battery rounds.

Ten of the 87 are files another SEST pack already ships: SEST Collection Fixes owns the NSM
(`usn_rgm_184a`), the three SM-3s and two Euromod SM-6s, and SEST Intercept Model owns Red
Storm Arsenal's three SM-6s and the Korean K-SAM II. Two packs cannot ship one path, and
forking the upstream copy instead would throw away what those packs fixed, so the owning
builder tags its own copy with the same `tag_ammunition()` from `integration/common/ras.py`.
This builder then checks both directions: every metered round a sibling owns must carry
...
- **4 modern hulls still name a store nothing in the collection defines** — `plan_cv_type_003`
  (`plan_f3200a`), `rnn_ffg_Van_Galen` (`wp_deploy_mine`), and `usn_lha_tripoli` /
  `wp_bdk_ivan_rogov_90` (amphibious `_spawner_` ids). They are patched anyway; the weapon
  holding the store has no round with or without this pack. `tools/check_dependencies.py`
  and `tools/check_weapon_employment.py` both report such inherited references without
  failing, and fail any this repo introduces.
- **About three hundred hulls are frozen at the export they were built from.** Every fork is a
  whole-file override, so an upstream update to one of these hulls does not reach the game
  until the pack is rebuilt. Rebuild after every export; see `docs/packaging-and-recovery.md`.
...
restoration is still needed, and no path is one another SEST pack already owns — except a
metered round, which the owning pack must already carry under the right category.
...
hulls), `integration/jmsdf-mogami/build_patch.py` (the Mogami's Mk141 canisters) and
`integration/allied-fixes/build_patch.py` (HMS Ocean), and its `tag_ammunition()` with
`integration/collection-fixes/build_patch.py` and `integration/intercept-model/build_patch.py`
(the metered rounds they own). Those packs own those files, and two packs shipping different
bytes at one path is an unconditional consolidation failure — so the transform is imported,
never duplicated.

`tools/check_pack_fidelity.py` proves every file this pack ships is its upstream plus only the
intended insertions, and `tools/check_reloadable.py` reports what still cannot be reloaded at
sea and why.
