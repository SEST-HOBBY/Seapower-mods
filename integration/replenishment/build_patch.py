...
Sea Power ships the whole mechanic and leaves it switched off. Outside
RE-power, the only `[SupplySystem1]` block on any hull in vanilla or in the
exported Workshop mods is the commented one on the Sacramento
...
separate reasons for it and neither is fixed by enabling the block:
  ...
  2. THE LAUNCHER GATE. A launcher fed by an `AssociatedMagazine=` refills
     ...
     vanilla and every exported mod the flag is set True only on land SAM
     TELs. No vessel anywhere sets it.
...
`Ammunition=` line, so without this pack not one VLS round on any of its
...
identical treatment without two packs ever shipping the same path. The same
goes for the metered rounds SEST Collection Fixes and SEST Intercept Model
ship: their builders tag them with ras.tag_ammunition, and stage_ammunition
here checks the tags against the rule.
...
MODERN_SOURCES = {
    ...
    "3789208859": "Euromod - South Korea Navy",
...
# surrogateescape, not the usual errors="replace". This pack ships some four
# hundred whole-file overrides, so every byte it does not deliberately change must
...
def write(rel, text):
    ...
    # rebuild there would diff every file against a Linux build.
...
def metered_rounds():
    ...
    judged = set()
    for ref in sorted(carried):
        # One verdict per ROUND, not per spelling. Units reference rounds in
        # more than one case - usn_ddg_burke_f3_concept hangs usn_rim-161D,
        # everything else usn_rim-161d - and the game, on case-insensitive
        # NTFS, reads both as one file. Judging each spelling would tag the
        # round twice and write two outputs that collide on the player's disk.
        # The id kept is the winning file's own stem, so the output overrides
        # the file it forks under that file's exact name.
        _, path = winning_ammo(ref)
        if path is None or path.stem.lower() in judged:
            continue
        judged.add(path.stem.lower())
        ammo_id = path.stem
...
def foreign_paths():
    """Relative paths already shipped by a sibling SEST pack -> that pack's file.

    consolidate_packs.py fails the build if two packs ship different bytes at
    one path, so this pack must never emit a file another pack owns. The map is
    computed from sibling output rather than listed, which is why this pack
    declares build_after on every pack that owns a vessel or a round it would
    otherwise write. Keys are case-folded, as the game's filesystem is.
    """
    owned = {}
    for pack in sorted((ROOT / "integration").glob("*/SEST_*")):
        ...
        for f in pack.rglob("*.ini"):
            owned[f.relative_to(pack).as_posix().lower()] = f
    return owned
...
def stage_clones(owned):
    """Eight new modern auxiliaries, cloned from vanilla donors (the Kaiser's
    donor is the Cold War Spanish pack's Teide)."""
...
_CATEGORY_LINE = re.compile(r"^SupplyCategory\s*=\s*([^\s/]+)", re.M)


def owner_label(path):
    """`SEST_Collection_Fixes` for integration/collection-fixes/SEST_.../x.ini."""
    return next(p.name for p in path.parents if p.name.startswith("SEST_"))


def stage_ammunition(owned):
    """Meter the heavy rounds, and put the four stripped rounds' keys back.

    A metered round another SEST pack already ships is the one case where this
    pack does not write the file. Shipping it too would fail consolidation, and
    forking the upstream copy instead would throw away whatever that pack fixed
    in it. So the owner tags its own copy with common.ras.tag_ammunition, and
    this stage proves the tag is there and is the category the rule derives
    here - a skip that is only allowed when the owner's copy is already
    metered, never a silent hole in the metering. The check runs both ways: a
    sibling round carrying a SEST_ category the rule no longer selects fails
    too, so a tag cannot outlive the reason for it.
    """
    tagged, restored, by_sibling = [], [], []
    tag, skipped, orphans, unpriced = metered_rounds()
    problems = []
    for ammo_id, category, mod in tag:
        rel = f"ammunition/{ammo_id}.ini"
        if rel.lower() in owned:
            theirs = owned[rel.lower()]
            declared = _CATEGORY_LINE.findall(read(theirs))
            if declared != [category]:
                problems.append(
                    f"{rel} is shipped by {owner_label(theirs)}, and the metering rule "
                    f"puts it under {category}, but that pack's copy declares "
                    f"{declared or 'no SupplyCategory'} - tag it in that pack's builder "
                    "with common.ras.tag_ammunition")
                continue
            by_sibling.append((ammo_id, category, owner_label(theirs)))
            continue
        _, path = winning_ammo(ammo_id)
        write(rel, tag_ammunition(read(path), category, ammo_id))
        tagged.append((ammo_id, category, mod))

    selected = {f"ammunition/{a}.ini".lower(): c for a, c, _ in tag}
    for rel, theirs in sorted(owned.items()):
        if not rel.startswith("ammunition/"):
            continue
        for category in _CATEGORY_LINE.findall(read(theirs)):
            if category.startswith("SEST_") and selected.get(rel) != category:
                problems.append(
                    f"{rel} in {owner_label(theirs)} declares SupplyCategory={category}, "
                    "which the metering rule does not select for it - drop the tag, "
                    "or find out why the rule stopped selecting the round")
    if problems:
        sys.exit("metering check failed:\n  " + "\n  ".join(problems))
    ...
    return tagged, by_sibling, restored, skipped, orphans, unpriced
...
def unused_rounds(ammo_ids):
    ...
    ones they are.
    """
    carried = {a.lower() for a in ship_referenced()}
    return [a for a in ammo_ids if a.lower() not in carried]
...
def main():
    ...
    tagged, by_sibling, restored, meter_skips, orphans, unpriced = stage_ammunition(owned)
    ...
    if by_sibling:
        print(f"  {len(by_sibling)} more metered in the sibling pack that ships them, "
              "tag verified:")
        for ammo_id, category, pack in by_sibling:
            print(f"      {ammo_id:<24} {category:<18} {pack}")
    ...
    for title, n in sorted(by_mod.items(), key=lambda kv: -kv[1])[:5]:
        print(f"      {n:>5} {title}")
    orphans = unused_rounds([r for r, _, _ in tagged + by_sibling])
    if orphans:
