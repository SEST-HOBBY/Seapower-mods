...
switched off. Outside RE-power, the ONLY `[SupplySystem1]` block on any hull
in vanilla or in the exported Workshop mods is commented out, on the
Sacramento:
...
integration/jmsdf-mogami/build_patch.py and integration/allied-fixes/
build_patch.py emit identical syntax and can never drift - and so that
integration/collection-fixes and integration/intercept-model, which ship
metered rounds of their own, tag them exactly as the metering rule would.
...
# [SupplySystem1] sections - the same duplicate-section defect the retired
# SEST_Zumwalt_CPS existed to fix - the day a supplier's fork source is a mod
# again (the Teide's already is).
...
def tag_ammunition(text, category, ammo_id):
    ...
    It lives here rather than in the RAS builder because not every metered
    round is that pack's to ship. A round another SEST pack already overrides -
    SEST_Collection_Fixes owns the NSM, the SM-3s and two SM-6s - cannot be
    shipped twice, so the owning builder applies this same function to its own
    copy, and the RAS builder checks that it did (stage_ammunition).
    """
    ...
    raise SystemExit(f"{ammo_id}: no [General] section and no #!alias directive to "
                     "anchor SupplyCategory to - re-check this file")
...
def make_reloadable(text):
    ...
    this transform not one VLS round on any hull it ships with one could
    ever be replenished, however many oilers were alongside.
