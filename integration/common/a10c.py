...
1. UNREGISTERED IR HEAD. [SensorSystem2], the Maverick head, declares no
   ModuleType=Sensor and no Mount. Of 2308 aircraft sensor blocks across this
   collection, vanilla included, 2263 declare ModuleType=Sensor; this is one
   of the 45 that do not, and usa_a-10a.ini from the same author is another.
   It is why the aircraft reads as carrying no infrared sensor at all.
...
def register_ir_head(text, where):
    ...
    if re.search(r"^ModuleType=Sensor", block, re.M):
        raise SystemExit(f"{where}: [SensorSystem2] now registers itself upstream - "
                         "drop this fix")
    body = block.rstrip("\n")
    # keep the blank line that separates the block from the next section
    fixed = body + "\nMount=Dummy\nModuleType=Sensor" + block[len(body):]
