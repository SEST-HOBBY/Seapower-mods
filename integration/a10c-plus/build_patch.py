...
"upgraded variant" carrying a targeting pod has to be its own unit id. The
same reason the replenishment pack clones hulls rather than adding fits.
...
Three defects are fixed in the clone, and the first two are fixed in the base
aircraft by SEST Allied Fixes as well, because they are bugs rather than
upgrades:

  1. [SensorSystem2], the Maverick IR head, carries no ModuleType=Sensor and
     no Mount. Of 2167 aircraft sensor blocks in this collection 2126 declare
     ModuleType=Sensor; this is one of the 41 that do not, and the A-10A from
     the same author is another. That is why the aircraft reads as having no
     infrared sensor at all.
