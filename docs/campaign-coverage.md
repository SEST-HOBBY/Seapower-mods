# SEST Southern Watch — mod coverage

Generated by `integration/campaign/build_pack.py`. Do not edit.

The campaign is 20 missions built from 279 placed units. Every mod in the canonical load order (`data/load-order.tokens.txt`) and every SEST source pack appears below, with the mechanism that makes the game read its files. Coverage is measured on the load order rather than on catalog status, because five enabled entries are catalogued deprecated and two WIP while still being required donors.

| class | meaning | mods |
|---|---|---|
| `unit` | places the unit; this mod wins its file | 129 |
...
| `chinese-navy-plan` | Chinese Navy (PLAN) | `unit` | plan_em_sovremenny | Warramunga's Shot |
...
| `dingtools-weapon-pack` | Dingtools Weapon Pack | `store` | raaf_f-35a / dts_aim-260 | Blind Horizon |
...
| `french-helicopter-package` | French Helicopter Package | `unit` | fr_as-565_sa | The Relief Ship |
...
| `jh-7a` | XIAN JH-7A (歼轰-7A 飞豹) | `unit` | plaaf_jh7a | Warramunga's Shot |
...
| `modern-us-navy` | Modern US Navy | `unit` | usn_ddg_burke_f2a_g4_2022 | Fujian's Shadow |
...
| `raaf-f-35a` | RAAF F-35A Lighting II | `squadron` | raaf_f-35a | Blind Horizon |
...
| `s-70b-2-seahawk` | [DEPRECATED] S-70B-2 Seahawk with AGM-114 'Hellfire' Missiles | `unit` | S-70B-2_Seahawk | Western Passage |
...
| `sea-lynx` | Sea Lynx | `unit` | fr_sea_lynx | Western Passage |
...
| `type-071-lpd` | PLAN Type 071 Amphibious Transport Dock | `unit` | plan_lpd_type_071 | Warramunga's Shot |
...
| `z-21` | AVIC HARBIN Z-21 | `unit` | pla_z21 | Warramunga's Shot |
...
| `SEST_Campaign` | SEST Southern Watch Campaign | `campaign` | this pack: it ships the campaign, its twenty missions and their briefings. It is what the coverage below is measured on, so it cannot place a unit to reach itself | - |
...
| `SEST_RAAF_Bases` | SEST RAAF Bases | `unit` | airbase_raaf_tindal | Blind Horizon |
| `SEST_RAAF_F-35A_JATM` | SEST RAAF F-35A JATM | `unit` | raaf_f-35a | Blind Horizon |
