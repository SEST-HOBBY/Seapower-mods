...
The campaign is 26 missions built from 375 placed units. Every mod in the canonical load order (`data/load-order.tokens.txt`) and every SEST source pack appears below, with the mechanism that makes the game read its files. Coverage is measured on the load order rather than on catalog status, because five enabled entries are catalogued deprecated and two WIP while still being required donors.
...
| `a-10c` | A-10C | `store` | usa_a-10c / usaf_alq-184_2 | The Long Perimeter |
...
| `SEST_A10C_Plus` | SEST A-10C+ | `unit` | usaf_a-10c_plus | The Long Perimeter |
