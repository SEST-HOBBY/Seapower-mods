...
|---|---|
| One continuous campaign, two chapters | One `campaign.ini`, 24 core missions plus a two-way optional branch; two mission-browser folders (*Southern Reach*, *Tasman Shield*) for mission-by-mission play |
...
| Replenishment is unproven | Every service is a *protected window*: the named hulls must be inside the service box when the window's clock runs, then withdraw (the `03 Lifeline` Trigger8 shape SW09 uses). No page says fuel crossed a hose |
...
**Opposition.** The bible's Southern Watch rule holds: a modern escort is dangerous because of the encounter, not because of a thousand-mile hypersonic. The Type 052D (reach 1,050 NM in the winning file) appears only in the two fleet actions and the finale, screened by the carrier, where the player's Hobart is expected. Type 054A / 056A (85 NM) are the everyday escorts. Russian hulls with Kalibr are used as submarines (VICTOR, SIERRA-TWO) whose reach is a ceiling the builder treats as a ceiling; the surface escort in the Bight is the vanilla Udaloy (27 NM), on purpose.
...
| SR03 | `SR03CrewRecovered` | victory | TS12 | the trawler that held the airlink's crew is revealed as the spoiler's spotter |
...
| SR12 | `SR12NetworkNamed` | classify the network | TS01, TS04 | tender and escort identified at start |
...
| Replenishment | `ran_aor_supply` | `SEST_RAN_Fleet` | Variant1 Supply, Variant2 Stalwart; no supply system in the file |
...
| Russian surface | `wp_bpk_udaloy` (`_vanilla`), `wp_vt_boris_chilikin` (`re-power-resupply`) | | Udaloy reach 27, deck 2 (Ka-27); the oiler |
...
| Merchants | `civ_ms_freighter_d`, `civ_ms_sealift_pacific`, `civ_ms_amra`, `civ_ms_andizhan` (`re-power-resupply`); `civ_ms_mairangi_bay` (`merchants-expanded`); `anl_ms_bulk`, `ran_ms_roro_a` (`auxilliary-merchant-pack`, armed: WeaponStatus=Hold always); vanilla `civ_ms_ivan_franko` (liner), `civ_ms_ritina` (tanker), `civ_ms_roro_a/b/c` (ferries), `civ_ms_bulk`, `civ_ms_car_carrier_a`, `civ_ms_encounter` | | |
...
Centre −54.3, 158.3. **HMAS Supply** (Variant1, authored, named) and **MV Coral Pioneer** at the Buckles Bay anchorage (−54.50, 158.99: `coastal=True`; 1 NM off the isthmus), the **Macquarie Island station** as a neutral `civ_radiostation` ashore (−54.499, 158.937), the player's escorts Generated 3 NM east, HELO and RECON slots (Hobart Airport blue, 813 NM). Service window: Supply and Coral Pioneer inside 5 NM of Supply's start when 30 minutes have run (stage, `after_minutes=30`, sets `SR04ServiceHeld`), then both to the withdrawal line 14 NM north (authored `at`, radius 12). Red: **VICTOR** 45 NM south-west closing at telegraph 4; **RV Akademik Fersman** (`civ_ms_kommunist`, red, Hold) 35 NM west as her tender; a **Bear-F** (`wp_tu-142m`, ASW fit, Hold, routed over the anchorage at 8,000 ft, unlimited fuel by the engine's rule) making a pass. Objectives: **Service** (main), **Supply** (protect), **Cargo** (protect Coral Pioneer), **Restraint** (`spare` the Bear and the tender: a reconnaissance flight is not an attack). Fatal: Supply, Coral Pioneer. Window: buy (+Wedgetail), repair, rearm.
...
- That Type 055 / 052D hypersonics are balanced. The 052D is in the three fleet-scale missions and nowhere else, and its reach is recorded.
