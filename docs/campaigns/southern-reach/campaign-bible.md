...
| P-8A Poseidon | `usn_p8` | Squadron3 (RAAF) | 55 | SR03 |
| MQ-4C Triton | `raaf_mq-4c_triton` | Squadron1 | 40 | SR03 |
...
| Collector / tender | `wp_agi_okean` (`_vanilla`), `civ_ms_kommunist` (`_vanilla`), `plan_aor_type901` and `plan_aor_type903a` (`SEST_Replenishment`) | | the Okean collector among `civ_fv_okean` trawlers; a merchant as a tender; the group's replenishment ships are the real classes - the Type 903A Taihu keeping the protection group fuelled in December, the Type 901 Hulunhu that comes south with Liaoning |
...
Centre −40.5, 151.5 (138 NM off Gabo Island). The group transits north-east into the Tasman: **Liaoning**, **a 054A**, **a 056A**, the Type 901 replenishment ship **Hulunhu** (`plan_aor_type901` Variant1), *Nan Hai 27* and **RV Akademik Fersman**, all Tight, routed 040. Player Generated with HELO, RECON and **CAP** (F-35A from **RAAF Base East Sale**, blue, 248 NM) — the first fighter cover of the campaign; **Wedgetail 05** fixed. Objectives: **Network** (main: classify Liaoning, Hulunhu and *Nan Hai 27*, min 3, then hold the shadowing line — arrive after classify); **Restraint** (`spare` everything red: shadow, do not start the Tasman war); **Flagship**. Writes `SR12NetworkNamed`. Window: buy (+F-35A), repair, rearm (Hobart). The chapter card that follows reads the ledger.
