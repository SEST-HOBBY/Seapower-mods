...
| 5 | SW03 `victory.after.intel` | Amphibious control reports the lifter inside the pickup area; no invented hover fix or measured loading |
...
| 7 | SW02 `reveal_if`, `O1BeaconFound` | Recovered recorder evidence supports classification; acquire a local contact and apply ROE |
| 8 | SW04 `reveal_if`, `O2KiwiPicture` | Kiwi 01's prior report supports correlation; no satellite fix of a submerged boat |
| 9 | SW06 `reveals.Picture.intel` | Sentry 06 / fusion-cell report names the Luda and Type 054A, with current detections still needed |
...
| 13 | SW11 `reveal_if`, `SW06NorthernGroupClassified` | Correlate the escort screen with earlier Sentry reporting; identify the carriers separately |
...
| 18 | SW O1 `denied` | Escort watch reports Salvor's arrival; no unassigned Triton or unobserved crane activity |
| 19 | SW D7 `denied` | Exercise control scores the Bear's simulated release; retain the 1988 exercise setting |
...
| 22 | SW D4 `brief` | Earlier surface reports cue Bear 02 and the withdrawing ships; assigned sensors update the assessment |
| 23 | SR `02_sitrep`, protection-group paragraph | Satellite and maritime-patrol reporting supports the formation assessment |
| 24 | SR `04_sitrep`, northbound formation | Triton and imagery provide a last reported formation, course and membership still to be checked |
| 25 | SR `03b_intsum`, ice-edge paragraph | Radar-satellite detections and naval emissions locate the search area; no invented recent fix from grounded aircraft |
...
| 31 | TS01 `reveal_if`, `SR12NetworkNamed` | Correlate Nan Hai 27 with the 14 January record; current transmissions remain unconfirmed |
| 32 | TS03 `reveal_if`, `TS02SubNamed`, and opening | Correlate TANGO's Cook Strait record with the reported rendezvous; no fabricated second satellite pass or guaranteed dive response |
| 33 | TS04 `reveal_if`, `SR12NetworkNamed` | Correlate the escort identity; locate Liaoning separately instead of inventing a live radar fix |
| 34 | TS12 `reveal_if`, `SR03CrewRecovered` | Use the 12 December search record to identify Nan Hai 27; distinguish an assessed spotting role from observed transmissions |
