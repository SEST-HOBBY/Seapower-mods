...
Navy pack, which the launcher fix leaves alone: the Italian pack is Cold War only, and the
French one is still catalogued as work in progress, so forking its hulls would freeze them
mid-build. Fixable in a line when that changes; say so.
