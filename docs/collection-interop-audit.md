...
**Update, 2026-09-24** — superseded by a composition, not an order move. U.S. Navy 2027's current `usn_mh-60r.ini` (re-exported 2026-09-20) no longer derives from the MH-60R mod's model: it draws United States Naval Aviation's sh60 mesh with `SubModel33=Modex` and `SubModel34=Emblem`, so the table written for that model is 3737267013's, and the order had since put 3590477166's `number` table above it. SEST Collection Fixes now ships `aircraft/usn_mh-60r_squadrons.ini` - 3737267013's 19 squadrons plus `Squadron20`, 816 Squadron RAN (`Nation=Australia`, `flag_australia`) - and 3737267013's names for Squadron1-3. The RAN hulls, the RAAF bases and the campaign embark Squadron20. See build-notes, "Side operations sail your ships; helicopters are helicopters".
