...
| [DEPRECATED] Anzac Class Frigate ⚠️ **DEPRECATED** | unknown (model by Jason Simpson, Simpson's Virtual Slipway, used with permission) | ship | KEEP - SEST_RAN_Fleet dependency. Measured with tools/check_mod_conflicts.py: 5 contestable files, of which the two ran_ffh_anzac vessel files collide head-on with SEST_RAN_Fleet and are won by SEST at tier 0. That means the REAL Anzac hull this mod adds is masked today: SEST's ran_ffh_anzac is a clone of Euromod's Type 23 (rn_ff_type23_mlu) carrying the NSM guidance fix, and it keeps winning the id. Rebasing SEST_RAN_Fleet's Anzac onto this mod as its donor is the fix; until then the model is installed but unseen. Also ships its own RGM-184A and RIM-162 ESSM rounds (2 files nothing else provides) and merges systems/sensors.ini. ApproximateVersion=0.4.0, well behind the 0.8.x collection. **Overlaps:** SEST_RAN_Fleet (ships the SAME two files: vessels/ran_ffh_anzac.ini and ran_ffh_anzac_variants.ini) |
