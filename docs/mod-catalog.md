...
| PLA & PLAN & PLAAF AEP | unknown | weapons | New subscription, catalogued 19 Sep 2026. An Anchor Chain expansion: 18 ammunition files that #!extend rounds owned by the PLA land-unit pack, Type 003, Modern PLAN Systems and the J-16 mod. An extend only applies if it outranks the copy it layers onto, so it sits in tier 2 above those mods; below them it silently does nothing. Move it down to keep the original PL-15/DF-21D/HQ-19 values. Its plan_yj-18e.ini extends a file no other mod provides, so that round has no base. **Requires:** anchor-chain (its ammunition files use the #!extend directive) |
